 ...
 * src/main/java/net/dv8tion/jda/api/hooks/AutoListener.java written
 * by MinnDevelopment and adapted for use within Sandra.
 */
class EventManager : net.dv8tion.jda.api.hooks.EventListener {

    private val listeners = HashMap<Class<*>, List<Pair<Any, Method>>>()
    ...
    fun register(vararg registrants: Any) {
        for (listener in registrants) {
            for (method in listener::class.java.declaredMethods) {
                ...
                val pairs = listeners.computeIfAbsent(method.parameterTypes[0]) { mutableListOf() }
                (pairs as MutableList).add(listener to method)
            }
        }
    }

    fun unregister(registrant: Any) = listeners.remove(registrant)
    ...
    fun handleEvent(event: Any) {
        scope.launch {
            for (entry in listeners) {
                if (entry.key.isInstance(event)) {
                    for (pair in entry.value) {
                        try {
                            pair.second.invoke(pair.first, event)
                        } catch (e: IllegalAccessException) {
                            logger.error("Failed to invoke ${pair.second} for event ${event::class}", e)
                        } catch (e: InvocationTargetException) {
                            logger.error("One of the event listeners threw an uncaught exception", e)
                        }
                    }
                }
            }
